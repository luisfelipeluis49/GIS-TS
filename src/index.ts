...
import type { Command, Gtag, GtagCommands } from "npm:@types/gtag.js@^0.0.20";
...
export class CappedMap {
  ...
  elementByKeyMap: Map<string, unknown>;
...
export function getLocationDistance(
  ...
  let halfChordLengthSquare =
    ...
    getCosinus(firstLatitude) * getCosinus(secondLatitude) *
      getSinus(longitudeOffset * 0.5) * getSinus(longitudeOffset * 0.5);

  let angularDistance = 2 *
    Math.atan2(
      getSquareRoot(halfChordLengthSquare),
      getSquareRoot(1 - halfChordLengthSquare),
    );
...
export function getRandomInteger(
  ...
  return getFloorInteger(
    firstInteger + getRandom() * (lastInteger - firstInteger + 1),
  );
}
...
export function getMapByCode(
  elementArray: Array<{ code: string; value: unknown }>,
  default_value = null,
): { [key: string]: unknown } | unknown {
  if (elementArray) {
    let elementMap: { [key: string]: unknown } = {};
...
export function getMapById(
  elementArray: Array<{ id: string; value: unknown }>,
...
interface ElementWithKey {
  [key: string]: string;
}

export function getMap(
  elementArray: Array<ElementWithKey>,
  keyName = "id",
  default_value: unknown = null,
): { [key: string]: ElementWithKey } | unknown {
  if (elementArray) {
    let elementMap: { [key: string]: ElementWithKey } = {};
...
export function getUnaccentedText(
  ...
  for (let characterIndex = 0; characterIndex < text.length; ++characterIndex) {
    unaccentedTextArray.push(
      getUnaccentedCharacter(text.charAt(characterIndex), languageCode),
    );
  }
...
export function getHexadecimalTextFromInteger(
  ...
  return parseInt(integer.toString()).toString(16);
}
  ...
  if (isBrowser) {
    ...
    for (
      ...
      text += String.fromCharCode(
        parseInt(hexadecimalText.slice(byteIndex, byteIndex + 2), 16),
      );
    }
...
export function getTuidFromHexadecimalText(
  ...
  return getBase64TextFromHexadecimalText(hexadecimalText).replaceAll("+", "-")
    .replaceAll("/", "_").replaceAll("=", "");
}

// ~~

export function getTuidFromText(
  text: string,
): string {
  if (text === "") {
    return "";
  } else {
    return getTuidFromHexadecimalText(md5(text).toString());
  }
}
...
export function getHexadecimalTextFromBase64Text(
  ...
  if (isBrowser) {
    ...
    for (
      ...
      hexadecimalText += ("0" + text.charCodeAt(characterIndex).toString(16))
        .slice(-2);
    }
...
export function getHexadecimalTextFromTuid(
  ...
  return getHexadecimalTextFromBase64Text(
...
export function getUuidFromHexadecimalText(
  hexadecimalText: string | ArrayBuffer,
): string {
  return (
...
export function getTimeUuid(): string {
...
export function getRandomTuid(): string {
...
export function getCurrentMillisecondCount(): number {
  if (isBrowser) {
    return globalThis.performance.timing.navigationStart +
      globalThis.performance.now();
  } else {
    let hrTime = process.hrtime();

    return parseInt((hrTime[0] * 1000 + hrTime[1] / 1000000).toString());
  }
}
...
export function getCurrentUniversalDateTime(): Date {
...
export function getSystemDate(
  systemDate: Date | string | undefined,
): Date {
  if (systemDate === undefined) {
    return new Date();
  } else if (typeof systemDate === "string") {
...
export function getValidFileName(
  ...
  return replaceIteratively(
    fileName.replace(invalidCharacterExpression, "_"),
    "__",
    "_",
  );
}
...
export function addFileExtensionPrefix(
  ...
  if (
    lastDotCharacterIndex >= 0 &&
    lastDotCharacterIndex > lastSlashCharacterIndex
  ) {
    return filePath.slice(0, lastDotCharacterIndex) + fileExtensionPrefix +
      filePath.slice(lastDotCharacterIndex);
  } else {
...
export function updateLanguageTag() {
  languageTag = getTrimmedLanguageTag(
    languageCode + "-" + countryCode + "-" + continentCode,
  );
}
...
export function getSubstitutionPrefix(): string {
...
export function getSubstitutionSuffix(): string {
...
export function matchesValueSpecifier(
  ...
  valueByNameMap: Map<string, string> | undefined,
): boolean {
  if (valueByNameMap !== undefined) {
    ...
    if (match) {
      ...
      let otherValue: string | number = match[3];

      if (
        valueByNameMap !== undefined &&
        valueName in valueByNameMap
      ) {
        let value = valueByNameMap.get(valueName);
        ...
        if (value !== undefined) {
          isOperator = (operator === "=" && value === otherValue) ||
            (operator === "<" && value < otherValue) ||
            (operator === "<=" && value <= otherValue) ||
            (operator === ">=" && value >= otherValue) ||
            (operator === ">" && value > otherValue) ||
            (operator === "<>" && value !== otherValue);
...
export function getSubstitutedText(
  text: string,
  valueByNameMap: Map<string, string>,
): string {
  if (valueByNameMap !== undefined) {
...
export function getTranslatedText(
  multilingualText: string,
  valueByNameMap: Map<string, string> | string | undefined,
  languageTag_: string,
  ...
  if (languageTag_ !== defaultLanguageTag) {
    for (
      let translatedTextIndex = translatedTextArray.length - 1;
      ...
      if (colonCharacterIndex >= 0) {
        if (
          matchesTranslationSpecifier(
            translatedText.slice(0, colonCharacterIndex),
            valueByNameMap as Map<string, string>,
            languageTag_,
          )
        ) {
          return getSubstitutedText(
            translatedText.slice(colonCharacterIndex + 1),
            valueByNameMap as Map<string, string>,
          );
        }
      }
...
export function getTranslationArray(
  multilingualText: string,
): Array<{ specifier: string; data: string }> {
  let translatedTextArray = multilingualText.split("¨");
...
export function getNextLanguageTag(
  languageTagArray: Array<string>,
  translationArray: Array<{ specifier: string }>,
): string {
  for (
    let languageTagIndex = 1;
...
export function getProcessedText(
  ...
  for (let processedDualTag of processedDualTagArray) {
    ...
    for (let partIndex = 0; partIndex + 1 < partCount; partIndex += 2) {
...
export function getProcessedMultilineTextBySlug(
  ...
  if (textBySlugMap.has(textSlug)) {
    let text = textBySlugMap.get(textSlug) || "";
    return getProcessedMultilineText(
      text,
      valueByNameMap,
      languageTag,
      defaultLanguageTag,
...
export function toggleGoogleAnalytics(
  ...
  if (trackingIsEnabled) {
    if (!googleAnalyticsTrackingIsEnabled) {
      ...
      googleAnalyticsTrackingScript.src =
        "https://www.googletagmanager.com/gtag/js?id=" + trackingId;

      document.head.appendChild(googleAnalyticsTrackingScript);

      dataLayer = dataLayer || [];
      gtag = (...argumentArray: GtagCommands[Command]) =>
        (dataLayer as Record<string, unknown>[]).push(...argumentArray);
...
export function addClass(
  element: { classList: { add: (className: string) => void } },
  className: string,
): {
  classList: {
...
export function toggleClass(
  element: {
    classList: {
      add: (className: string) => void;
      contains: (className: string) => unknown;
      remove: (className: string) => void;
    };
  },
  className: string,
