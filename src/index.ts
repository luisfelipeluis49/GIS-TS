...
import type { Gtag, GtagCommands, Command } from "npm:@types/gtag.js@^0.0.20";
...
export let
    ...
    processedLineTagArray: Array< { name: string, openingDefinition: string, closingDefinition: string } > = [],
    processedDualTagArray: Array< { name: string, openingDefinition: string, closingDefinition: string } > = [],
    ...
    )
{
    if ( isBrowser )
    {
        return globalThis.performance.timing.navigationStart + globalThis.performance.now();
    }
    else
    {
    ...
    ): string
{
    ...
    if ( languageTag_ !== defaultLanguageTag )
    {
              ...
              --translatedTextIndex )
        {
            ...
            {
                if ( matchesTranslationSpecifier( translatedText.slice( 0, colonCharacterIndex ), valueByNameMap as Map<string, string>, languageTag_ ) )
                {
                    return getSubstitutedText( translatedText.slice( colonCharacterIndex + 1 ), valueByNameMap as Map<string, string> );
                }
            }
        }
    }

    return getSubstitutedText( translatedTextArray[ 0 ], valueByNameMap as Map<string, string> );
}
    ...
    ): string
{
    if ( textBySlugMap.has( textSlug ) )
    {
        ...
        return getLocalizedText( text , valueByNameMap, languageTag, defaultLanguageTag );
    }
    else
    {
        logWarning( 'Missing text slug : ' + textSlug );
    ...
    )
{
    ...
    {
        let text = textBySlugMap.get( textSlug ) || '';
        return getProcessedMultilineText( text, valueByNameMap, languageTag, defaultLanguageTag );
    }
    else
    {
        logWarning( 'Missing text slug : ' + textSlug );
...
{
    let dataLayer = ( window as unknown as { dataLayer?: Record<string, unknown>[] } ).dataLayer;
    ...
    if ( trackingIsEnabled )
    {
        if ( !googleAnalyticsTrackingIsEnabled )
        {
            ...
            dataLayer = dataLayer || [];
            gtag = ( ...argumentArray: GtagCommands[ Command ] ) => ( dataLayer as Record<string, unknown>[]).push( ...argumentArray );
